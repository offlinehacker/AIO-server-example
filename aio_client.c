...
void IssueReadOperation(struct aiocb* pAiocb, char* pBuffer)
{
    ...
    pAiocb->aio_sigevent.notify_function = aio_read_completion_handler;
    pAiocb->aio_sigevent.notify_attributes = NULL;
    pAiocb->aio_sigevent.sigev_value.sival_ptr = &my_aiocb;
...
void IssueWriteOperation(struct aiocb* pAiocb, char* pBuffer)
{
    ...
    /* Link the AIO request with a thread callback */
    pAiocb->aio_sigevent.sigev_notify = SIGEV_THREAD;
    pAiocb->aio_sigevent.notify_function = aio_write_completion_handler;
    pAiocb->aio_sigevent.notify_attributes = NULL;
    pAiocb->aio_sigevent.sigev_value.sival_ptr = &my_aiocb;
